...
import { createSkillFromName, setSkillActive, upsertSkill } from './service';
...
export async function createSkillFromNameAction(input: { name: string }) {
  return toActionResult(() => createSkillFromName(input.name));
}
