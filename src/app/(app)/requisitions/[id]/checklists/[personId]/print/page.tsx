      ...
      <main className="mx-auto my-6 max-w-3xl bg-white p-10 shadow-sm print:my-0 print:max-w-none print:p-0 print:shadow-none">
        ...
        {checklist?.notes && (
          <section className="mt-6">
            ...
            <p className="mt-2 whitespace-pre-wrap rounded border border-slate-300 p-3 text-sm">
              {checklist.notes}
            </p>
          </section>
        )}
...
function MetaRow({ label, value }: { label: string; value: string }) {
  return (
    <tr className="border border-slate-300">
      ...
      <td className="border-l border-slate-300 px-3 py-2">{value}</td>
    </tr>
  );
}
  ...
  return (
    <section className="mt-6">
      ...
      <table className="w-full border-collapse text-sm">
        <thead>
          <tr>
            <th className="w-[55%] bg-slate-100 px-3 py-2 text-left font-semibold uppercase tracking-wider text-xs text-slate-700 border border-slate-300">
              Requirement
            </th>
            <th className="w-[15%] bg-slate-100 px-3 py-2 text-center font-semibold uppercase tracking-wider text-xs text-slate-700 border border-slate-300">
              Answer
            </th>
            <th className="bg-slate-100 px-3 py-2 text-left font-semibold uppercase tracking-wider text-xs text-slate-700 border border-slate-300">
              Note
            </th>
          </tr>
        </thead>
        <tbody>
          {fields.map((f) => {
            ...
            return (
              <tr key={f.key} className="border border-slate-300">
                <td className="border border-slate-300 px-3 py-2">{f.label}</td>
